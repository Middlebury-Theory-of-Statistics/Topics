...
career <- Batting %>% 
  ...
  summarize(H = sum(H), AB=sum(AB), `Last Year`=max(yearID)) %>%
  mutate(Avg = H / AB) %>% 
  ungroup() %>% 
  select(-playerID)
```
...
```{r, message=FALSE, warning=FALSE, echo=FALSE}
quantile(career$Avg, probs=c(0.25, 0.5, 0.75)) %>% 
  round(3)
```
...
These are among batters with the best averages. Are they the best?
...
prior_plot <- ggplot(data=career_filtered, aes(x=Avg)) + 
  ...
  # stat_function(fun = dbeta, colour = "red", args=list(shape1=alpha0, shape2=beta0), size=2) +
...
career %>% 
  ...
  select(Name, `Last Year`, H, AB, Shrunk_Avg) %>% 
  datatable(options = list(pageLength=5, dom = 'tp'))
```
